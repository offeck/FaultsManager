...
class faultshower(myframe):
    def __init__(self, parent, openfault, c):
        ...
        self.deletebutton = tk.Button(self, text='X', command=self.ondeletebutton, fg='#DDDDDD', bg='#06113C')
        self.deletebutton.grid(row=c, column=0)
        self.summarybutton = tk.Button(self, text=self.openfault.getsummary(), command=self.onsummarybutton,bg='#06113C',fg='#FF8C32')
        self.summarybutton.grid(row=c, column=1)
        self.uptimelabel = tk.Label(self, text=self.openfault.getuptime(), bg='#FF8C32',fg='#DDDDDD')
...
class closefaultframe(myframe):
    def __init__(self, parent, openfault, prevframe=None):
        ...
        self.openfault = openfault
        tk.Label(self, text=self.openfault.getexpandedsummary(),fg="#FF8C32", bg="#DDDDDD").grid(columnspan=2,pady=10)
        self.entries = []
        # self.translated = True
        for c,field in enumerate(openfault.getfields(['devicename','component','description','techcomment'],True).items()):
            print(field)
            tk.Label(self,text=field[0],bg='#FF8C32',fg='#DDDDDD').grid(row=c+1,column=1)
            print(get_keys_from_value(engtoheb,field[0]))
            self.entries.append((get_keys_from_value(engtoheb,field[0])[0],tk.Entry(self,justify='right',bg='#DDDDDD')))
            self.entries[-1][1].insert(0,field[1])
            if field[1] == "אחר":
                self.entries[-1][1].configure(fg='red')
            self.entries[-1][1].grid(row=c+1,column=0,pady=(0,7))
        tk.Button(self, text='סגור תקלה',fg="#FF8C32", bg="#DDDDDD", command=self.onbuttonclose).grid(columnspan=2,pady=(0,7))
...
class openfaultframe(myframe):
    def __init__(self, stage, options, parent ,prevframe=None, data = {}):
        ...
        self.stage = stage
        self.data = data
        tk.Label(master=self,text=engtoheb[stage],fg="#FF8C32", bg="#DDDDDD",font=("Arial", 10)).grid(pady=10)
...
if __name__ == '__main__':
    ...
    tk.Label(root, text=config['user'],font=("Arial", 16),bg='#06113C',fg='#FF8C32').pack(padx=55)
